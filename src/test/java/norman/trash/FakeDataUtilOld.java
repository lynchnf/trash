...
public class FakeDataUtilOld {
    private static final Logger LOGGER = LoggerFactory.getLogger(FakeDataUtilOld.class);
    ...
    public static Acct buildAcct(long id, String name, AcctType type, String ofxFid) {
        return buildAcctImpl(id, name, type, ofxFid, null);
    }

    public static Acct buildAcct(long id, Map<String, Acct> acctMap) {
        return buildAcctImpl(id, null, null, null, acctMap);
    }

    private static Acct buildAcctImpl(long id, String name, AcctType type, String ofxFid, Map<String, Acct> acctMap) {
        Acct acct = new Acct();
        acct.setId(id);
        if (type == null) {
        ...
        if (name == null) {
            do {
                ...
                fullName = firstPartName + " " + lastPartName;
            } while (acctMap.keySet().contains(fullName));
        } else {
            firstPartName = WORDS[RANDOM.nextInt(WORDS.length)];
            fullName = name;
        }
        acct.setName(fullName);

        acct.setAddressName(fullName + " Inc.");
    ...
    public static Stmt buildBeginStmt(long id, Acct acct) {
        return buildBeginStmt(id, null, acct);
    }

    public static Stmt buildBeginStmt(long id, Date closeDate, Acct acct) {
        if (closeDate == null) {
            Calendar cal = Calendar.getInstance();
            cal.set(Calendar.MILLISECOND, 0);
            cal.set(Calendar.SECOND, 0);
            ...
            return buildStmtImpl(id, acct, closeDate);
        }
    }
